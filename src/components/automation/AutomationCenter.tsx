...
import { Play, Square, Pause, Zap, Clock, CheckCircle, XCircle, Globe, Cpu, Key, RefreshCw } from 'lucide-react'

interface LogEntry { id: string; level: string; message: string; timestamp: string; jobType?: string }
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await fetch('/api/automation/trigger', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'trigger', plateNumber, count: targetCount, accountId: 'default' }),
      })
      const data = await res.json()
      ...
      <Tabs defaultValue="trigger">
        ...
        <TabsContent value="workers" className="pt-4">
          <div className="grid gap-3 sm:grid-cols-3">
            {[{ name: 'ورکر-۱', status: botRunning ? 'active' : 'idle' }, { name: 'ورکر-۲', status: 'idle' }, { name: 'ورکر-۳', status: 'idle' }].map((w) => (
              <Card key={w.name}><CardContent className="p-4 space-y-3">
                ...
                <p className="text-xs text-muted-foreground">CPU: -- | حافظه: --</p>
              </CardContent></Card>
            ))}
          </div>
        </TabsContent>
              ...
              {logs.length === 0 ? <p className="text-center text-muted-foreground py-8">لاگی موجود نیست</p> : logs.map((log) => (
                <div key={log.id} className="flex items-center gap-3 rounded px-3 py-1.5 hover:bg-muted/30">
                  ...
                  <Badge variant={log.level === 'error' ? 'destructive' : log.level === 'success' ? 'default' : 'secondary'} className="w-14 justify-center text-[10px]">
                    {log.level === 'error' ? 'خطا' : log.level === 'success' ? 'موفق' : log.level === 'warn' ? 'هشدار' : 'اطلاعات'}
                  </Badge>
